...
export class TableRankerService {
    ...
    private detectSpatialIntent(text = ''): boolean {
        ...
        // detectar intención espacial basada en viajes/logística
        return /(mapa|ubicación|latitud|longitud|distancia|cerca de|origen|destino)/i.test(t);
    }
    ...
    rankTables(
        ...
        for (const [fullName, meta] of Object.entries(schema.tables)) {
            ...
            // 1️⃣ Aliases
            const aliasList = (aliases[fullName] || []).map(a => this.normalizeName(a));
            ...
            // 3️⃣ Tokens vs column names
            for (const t of tokens) {
                const tn = this.normalizeName(t);
                for (const c of columns) {
                    if (this.normalizeName(c).includes(tn)) score += columnTokenBoost;
                }
            }

            // 4️⃣ N-grams similarity
